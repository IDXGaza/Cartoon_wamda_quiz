...
const LibraryScreen: React.FC<Props> = ({ onPlaySet, onClose }) => {
  ...
  const confirmDelete = async (id: string) => {
    playSound('click');
    const path = `saved_sets/${id}`;
    try {
      await deleteDoc(doc(db, 'saved_sets', id));
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !topic.trim() || !auth.currentUser) return;
    ...
    const path = 'saved_sets';
    try {
      const isGridMode = mode === GameMode.HEX_GRID || mode === GameMode.GRID;
      const requiredCount = mode === GameMode.HEX_GRID ? 25 : (mode === GameMode.GRID ? 20 : numQuestions);
      
      // Auto-detect categories if topic is comma-separated
      let detectedCategories: string[] | undefined = undefined;
      let finalTopic = topic;
      
      if (mode === GameMode.GRID && (topic.includes('،') || topic.includes(','))) {
        detectedCategories = topic.split(/[،,]/).map(s => s.trim()).filter(Boolean);
        if (detectedCategories.length >= 2) {
          // Keep top 5 categories
          detectedCategories = detectedCategories.slice(0, 5);
        } else {
          detectedCategories = undefined;
        }
      }
      ...
      await setDoc(doc(db, 'saved_sets', setId), newSet);
      
      const setWithId: SavedSet = { id: setId, ...newSet };
      setSavedSets([setWithId, ...savedSets]);
      setIsCreating(false);
      // Reset form
      setName('');
      setTopic('');
      ...
      if (err instanceof Error && err.message.startsWith('{')) {
        setError("فشلت عملية الحفظ بسبب قيود الخصوصية.");
      } else {
  ...
  return (
    <div className="vintage-panel p-4 md:p-12 max-w-5xl mx-auto animate-fade-up relative overflow-hidden rounded-3xl md:rounded-[3rem] border-4 border-[var(--color-ink-black)] shadow-[6px_6px_0px_var(--color-ink-black)] md:shadow-[8px_8px_0px_var(--color-ink-black)] bg-[var(--color-bg-cream)] mt-4 md:mt-0">
      <div className="flex justify-between items-center mb-6 md:mb-10 bg-[var(--color-primary-gold)] p-4 md:p-6 rounded-2xl md:rounded-[2rem] border-4 border-[var(--color-ink-black)] shadow-[4px_4px_0px_var(--color-ink-black)] flex-wrap gap-4">
        <h2 className="text-2xl md:text-5xl font-display text-[var(--color-ink-black)] flex items-center gap-2 md:gap-4">
          <CartoonBook size={32} className="md:w-[48px] md:h-[48px]" />
        ...
        <button 
          ...
          className="w-10 h-10 md:w-14 md:h-14 bg-[var(--color-primary-red)] text-white rounded-xl md:rounded-2xl flex items-center justify-center transition-transform hover:scale-110 border-2 md:border-4 border-[var(--color-ink-black)] shadow-[2px_2px_0px_var(--color-ink-black)] md:shadow-[4px_4px_0px_var(--color-ink-black)] active:translate-y-1 active:shadow-none"
        >
          {selectedSetToPlay ? <CartoonSkip size={24} className="md:w-[32px] md:h-[32px] rotate-180" /> : <CartoonX size={24} className="md:w-[32px] md:h-[32px]" />}
        </button>
      </div>

      {selectedSetToPlay ? (
        <div className="space-y-6 md:space-y-8">
          <div className="bg-[var(--color-off-white)] p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border-4 border-[var(--color-ink-black)] shadow-[inner_4px_4px_0px_rgba(0,0,0,0.1)]">
            <h3 className="text-xl md:text-3xl font-display text-[var(--color-ink-black)] mb-4 md:mb-6">المجموعة: {selectedSetToPlay.name}</h3>
            <div className="flex flex-wrap gap-2 md:gap-4">
              <span className="bg-[var(--color-primary-blue)] text-white px-4 md:px-6 py-2 rounded-xl border-2 border-[var(--color-ink-black)] font-display text-sm md:text-xl shadow-[2px_2px_0px_var(--color-ink-black)]">{getModeLabel(selectedSetToPlay.mode)}</span>
              <span className="bg-[var(--color-primary-gold)] text-[var(--color-ink-black)] px-4 md:px-6 py-2 rounded-xl border-2 border-[var(--color-ink-black)] font-display text-sm md:text-xl shadow-[2px_2px_0px_var(--color-ink-black)]">{selectedSetToPlay.numQuestions} سؤالاً</span>
              <span className="bg-[var(--color-primary-red)] text-white px-4 md:px-6 py-2 rounded-xl border-2 border-[var(--color-ink-black)] font-display text-sm md:text-xl shadow-[2px_2px_0px_var(--color-ink-black)]">{getDifficultyLabel(selectedSetToPlay.difficulty)}</span>
            </div>
          </div>

          <div className="space-y-4 md:space-y-6 bg-[var(--color-off-white)] p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border-4 border-[var(--color-ink-black)] shadow-[inner_4px_4px_0px_rgba(0,0,0,0.1)]">
            <h3 className="text-xl md:text-2xl font-display text-[var(--color-bg-dark)] mb-2 md:mb-4">المتنافسون</h3>
              ...
              {playersConfig.map((p, i) => (
                <div key={i} className="flex gap-4 items-center bg-[var(--color-bg-cream)] p-4 rounded-2xl border-4 border-[var(--color-ink-black)] shadow-[4px_4px_0px_var(--color-ink-black)]">
                  ...
                  {playersConfig.length > 2 && (
                    <button type="button" onClick={() => setPlayersConfig(playersConfig.filter((_, idx) => idx !== i))} className="w-10 h-10 md:w-12 md:h-12 bg-[var(--color-primary-red)] text-white rounded-xl border-2 md:border-4 border-[var(--color-ink-black)] shadow-[2px_2px_0px_var(--color-ink-black)] hover:scale-110 transition-transform flex items-center justify-center shrink-0">
                      <CartoonX size={20} className="md:w-[24px] md:h-[24px]" />
                    </button>
                  )}
                </div>
              ))}
            ...
            }} className="w-full py-3 md:py-4 border-2 md:border-4 border-dashed border-[var(--color-ink-black)] rounded-xl md:rounded-2xl text-[var(--color-bg-dark)] text-lg md:text-2xl font-display hover:bg-[var(--color-primary-gold)]/10 transition-all flex items-center justify-center gap-2 md:gap-4">
              <CartoonPlus size={24} className="md:w-[32px] md:h-[32px]" />
          ...
          <button 
            ...
            className="vintage-button w-full py-6 md:py-8 rounded-2xl md:rounded-[2.5rem] text-2xl md:text-4xl font-display flex items-center justify-center gap-4 md:gap-6 bg-[var(--color-primary-green)]"
          >
            <span>بدء اللعب</span>
            <CartoonRocket size={32} className="md:w-[48px] md:h-[48px]" />
          </button>
        </div>
      ) : !isCreating ? (
