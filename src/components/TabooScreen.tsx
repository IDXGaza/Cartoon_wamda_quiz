...
const TabooScreen: React.FC<Props> = ({ config, questions, players, onFinish }) => {
  const [roomId] = useState(() => {
    ...
    return params.get('roomId') || Math.random().toString(36).substring(2, 8).toUpperCase();
  });
  ...
  const isHost = auth.currentUser?.uid === roomState?.hostId;
  ...
  const nextWord = async (scoreChange: number) => {
    ...
    if (isHosting && questions && currentQuestionIndex < questions.length - 1) {
  ...
  const joinUrl = `${window.location.origin}${window.location.pathname}#/join?mode=taboo&roomId=${roomId}`;
  ...
  return (
    <div className="h-screen p-4 bg-[var(--color-bg-cream)]">
        <div className="vintage-panel p-6 rounded-[2rem] border-4 border-[var(--color-ink-black)] bg-white text-center mb-6 shadow-[6px_6px_0px_var(--color-ink-black)]">
            <h1 className="text-2xl font-display mb-4 text-[var(--color-ink-black)]">{currentWord}</h1>
            <div className="grid grid-cols-2 gap-2">
                {tabooWords.map((word: string) => (
                    <div key={word} className="bg-[var(--color-primary-red)]/10 text-[var(--color-primary-red)] py-2 rounded-lg font-bold text-sm border-2 border-[var(--color-primary-red)]">
                        {word}
                    </div>
                ))}
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
            {remotePlayers.map(player => (
                <div key={player.id} className="vintage-panel p-4 rounded-2xl border-4 border-[var(--color-ink-black)] bg-white shadow-[4px_4px_0px_var(--color-ink-black)] text-center">
                    <p className="text-sm font-bold text-[var(--color-ink-black)]">{player.name}</p>
                    <p className="text-3xl font-display text-[var(--color-primary-green)]">{player.score || 0}</p>
                </div>
            ))}
        </div>
    </div>
