...
import { collection, doc, onSnapshot, setDoc, updateDoc, deleteDoc, query, orderBy, increment } from 'firebase/firestore';
...
const BuzzerScreen: React.FC<Props> = ({ config, questions, onFinish }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      unsubRoom();
      unsubPlayers();
      // We don't delete the room immediately on unmount to prevent race conditions
      // during refreshes or brief disconnects.
    };
  }, [roomId]);
  ...
  const handleJudge = async (correct: boolean) => {
    ...
    try {
      if (correct) {
        ...
        playSound('wrong');
        // Subtract half points but don't go below 0 (wait, increment can take negative, but how to cap at 0 in one call?)
        // Firestore doesn't support min/max in increment. 
        // But for negative, we can just subtract. 
        // If we want to cap at 0, we might need a transaction or just let it go slightly negative if that's okay.
        // Usually in trivia, negative scores are fine.
        await updateDoc(playerRef, { score: increment(-(points / 2)) });
      }
              ...
              <div className="bg-[var(--color-off-white)] p-4 rounded-2xl border-4 border-[var(--color-ink-black)]">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {remotePlayers.length === 0 ? (
                    ...
                    remotePlayers.map(p => {
                      const isDark = isColorDark(p.color || '#000000');
                      return (
                        <div key={p.id} className={`px-4 py-2 rounded-xl border-2 border-[var(--color-ink-black)] font-bold shadow-[2px_2px_0px_var(--color-ink-black)] ${isDark ? 'text-[var(--color-off-white)]' : 'text-[var(--color-ink-black)]'}`} style={{ backgroundColor: p.color }}>
                          {p.name}
                        </div>
                      );
                    })
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 md:space-y-8 animate-fade-in px-2 md:px-0">
      {/* Question Card */}
      <div className="vintage-panel p-4 md:p-10 rounded-3xl md:rounded-[3rem] border-4 border-[var(--color-ink-black)] shadow-[6px_6px_0px_var(--color-ink-black)] md:shadow-[12px_12px_0px_var(--color-ink-black)] bg-[var(--color-bg-cream)] relative overflow-hidden">
        ...
        {buzzedPlayer && (
          <div className="mt-4 md:mt-8 animate-bounce">
            <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-[2rem] border-4 border-[var(--color-ink-black)] shadow-[4px_4px_0px_var(--color-ink-black)] md:shadow-[8px_8px_0px_var(--color-ink-black)] flex items-center justify-center gap-4 md:gap-6 flex-wrap">
        ...
        <div className="mt-8 md:mt-10 flex flex-col gap-4">
          {!showAnswer ? (
            <button 
              ...
              className="vintage-button w-full py-4 md:py-6 rounded-2xl text-xl md:text-3xl font-display bg-[var(--color-primary-gold)] flex items-center justify-center gap-3 md:gap-4"
            >
              <CartoonEye size={32} className="md:w-[40px] md:h-[40px]" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <button 
                ...
                className="vintage-button py-4 md:py-6 rounded-2xl text-xl md:text-3xl font-display bg-[var(--color-primary-green)] text-white flex items-center justify-center gap-3 md:gap-4"
              >
                <CartoonCheck size={32} className="md:w-[40px] md:h-[40px]" />
              ...
              <button 
                ...
                className="vintage-button py-4 md:py-6 rounded-2xl text-xl md:text-3xl font-display bg-[var(--color-primary-red)] text-white flex items-center justify-center gap-3 md:gap-4"
              >
                <CartoonX size={32} className="md:w-[40px] md:h-[40px]" />
      ...
      {/* Leaderboard */}
      <div className="vintage-panel p-6 md:p-8 rounded-3xl md:rounded-[2.5rem] border-4 border-[var(--color-ink-black)] shadow-[6px_6px_0px_var(--color-ink-black)] md:shadow-[8px_8px_0px_var(--color-ink-black)] bg-[var(--color-off-white)]">
