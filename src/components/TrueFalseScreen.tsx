...
const TrueFalseScreen: React.FC<Props> = ({ config, questions = [], players: initialPlayers, onFinish }) => {
  ...
  if (!questions || questions.length === 0 || !questions[currentIndex]) {
    return (
      <div className="flex flex-col items-center justify-center p-8 vintage-panel max-w-lg mx-auto mt-20 text-center">
        <CartoonAlert size={64} className="mb-4 text-[var(--color-primary-red)]" />
        <h2 className="text-3xl font-black text-[var(--color-ink-black)] mb-4 vintage-text">لا توجد أسئلة!</h2>
        <p className="text-[var(--color-bg-dark)] font-bold mb-8">
          عذراً، لم نتمكن من العثور على أو تحميل أسئلة لهذا القسم. حاول العودة واختيار موضوع آخر.
        </p>
        <button 
          onClick={() => window.location.reload()}
          className="vintage-button bg-[var(--color-primary-gold)] px-8 py-4 text-xl font-black"
        >
          العودة للرئيسية
        </button>
      </div>
  ...
  const handleAnswer = (answer: string) => {
    ...
    const normalize = (str: string) => {
      ...
      if (n === 'صواب' || n === 'حقيقي' || n === 'true' || n === 'صح') return 'صح';
      if (n === 'خطأ' || n === 'خاطئ' || n === 'false' || n === 'خطا') return 'خطأ';
      return n;
    };

    const correct = normalize(currentQuestion.answer) === normalize(answer);
            ...
            {gameState === 'question' ? (
              ...
              >
                ...
                {currentQuestion.explanation && (
                  <div className="p-6 md:p-8 bg-white/50 border-4 border-[var(--color-ink-black)] rounded-3xl text-right relative shadow-[6px_6px_0px_var(--color-ink-black)]">
                    <div className="absolute -top-4 right-6 bg-[var(--color-accent-sky)] text-[var(--color-ink-black)] px-4 py-1.5 rounded-xl font-black border-2 border-[var(--color-ink-black)] text-sm md:text-base flex items-center gap-2">
                      <CartoonEye size={18} />
                      تفسير المعلومة
                    </div>
