    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 md:p-6 bg-[var(--color-bg-cream)] text-[var(--color-ink-black)]">
      ...
      >
        ...
        <div className="bg-[var(--color-off-white)] p-6 md:p-10 rounded-[1.5rem] md:rounded-[2.5rem] border-4 border-[var(--color-ink-black)] mb-6 md:mb-10 shadow-[inner_4px_4px_0_rgba(0,0,0,0.1)]">
          ...
          {showWordToActor && (
            <div className={`flex items-center justify-center gap-3 md:gap-4 mt-10 p-3 md:p-4 rounded-xl md:rounded-2xl border-2 ${timeLeft <= 10 ? 'bg-[var(--color-primary-red)]/10 border-[var(--color-primary-red)]' : 'bg-[var(--color-primary-gold)]/10 border-[var(--color-primary-gold)]'}`}>
              <CartoonTimer size={32} className={`${timeLeft <= 10 ? 'text-[var(--color-primary-red)] animate-ping' : 'text-[var(--color-primary-gold)]'} md:w-12 md:h-12`} />
              <p className={`text-2xl md:text-5xl font-display ${timeLeft <= 10 ? 'text-[var(--color-primary-red)]' : 'text-[var(--color-bg-dark)]'}`}>الوقت: {timeLeft} ثانية</p>
            </div>
          )}
        </div>
        ...
        <button 
          onClick={() => {
            ...
            if (!showWordToActor) setTimeLeft(initialTime);
          }}
