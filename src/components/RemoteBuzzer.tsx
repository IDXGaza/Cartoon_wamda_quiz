...
import { doc, onSnapshot, setDoc, updateDoc, deleteDoc, serverTimestamp, runTransaction, increment } from 'firebase/firestore';
...
const RemoteBuzzer: React.FC = () => {
  ...
  const handleBuzz = async () => {
    ...
    try {
      ...
      // Use Firestore transaction to handle race conditions server-side
      await runTransaction(db, async (transaction) => {
        ...
        // If someone else already buzzed according to server state, abort
        if (data.buzzedPlayerId) {
          throw new Error("Someone else buzzed first");
        }

        // We are first! Update the state.
      ...
      if (err.message === "Someone else buzzed first") {
        console.log("لقد سبقك أحد المتسابقين!");
        // The roomState listener will update the UI accordingly
      } else {
        setIsBuzzed(false); // Reset on unexpected failure
          ...
          <form onSubmit={handleJoin} className="space-y-6">
            <div className="bg-[var(--color-primary-gold)]/10 p-4 rounded-2xl border-2 border-dashed border-[var(--color-primary-gold)] text-center mb-4">
              ...
              <input 
                ...
                className="text-2xl font-display text-[var(--color-primary-blue)] tracking-widest bg-transparent border-none text-center w-full focus:outline-none"
