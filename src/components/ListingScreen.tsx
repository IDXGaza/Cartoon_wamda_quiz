    ...
    <div className="flex flex-col items-center justify-center h-screen p-6 bg-[var(--color-bg-cream)]" dir="rtl">
      {gameState === 'bidding' ? (
        ...
        <div className="w-full max-w-lg text-center space-y-6">
          <h2 className="text-3xl font-display">موضوع: {currentQuestion?.text}</h2>
          <p className="text-2xl">المطلوب: {currentQuestion?.answer}</p>
          <p className="text-7xl font-display text-[var(--color-primary-red)]">{timeLeft}</p>
          <div className="grid grid-cols-2 gap-4">
            <button onClick={() => { setCorrect(c => c + 1); playSound('correct'); setCurrentIndex(i => Math.min(questions.length - 1, i + 1)); }} className="p-6 bg-green-500 text-white text-2xl font-bold rounded-2xl border-4 border-black">صح</button>
            <button onClick={() => { setIncorrect(i => i + 1); playSound('wrong'); setCurrentIndex(i => Math.min(questions.length - 1, i + 1)); }} className="p-6 bg-red-500 text-white text-2xl font-bold rounded-2xl border-4 border-black">خطأ</button>
          </div>
        </div>
      ) : (
