  ...
  return (
    <div className="vintage-panel p-12 md:p-20 max-w-2xl mx-auto text-center animate-fade-up relative overflow-hidden rounded-[3rem]">
      <div className="mb-14 relative z-10">
        ...
        <p className="text-[var(--color-ink-black)] font-display text-lg bg-[var(--color-primary-gold)] inline-block px-8 py-2 rounded-2xl border-4 border-[var(--color-ink-black)] shadow-[4px_4px_0_var(--color-ink-black)]">أبطال المسابقة</p>
      </div>
        ...
        {sorted.map((p, idx) => (
          <div key={p.id} className={`flex items-center justify-between p-6 rounded-[2rem] transition-all border-4 border-[var(--color-ink-black)] shadow-[6px_6px_0px_var(--color-ink-black)] ${idx === 0 ? 'bg-[var(--color-primary-gold)]/20 scale-105 z-10 relative' : 'bg-[var(--color-off-white)]'}`}>
            ...
            <div className="text-right flex flex-col items-end">
              ...
              <span className="text-xs font-display text-[var(--color-bg-dark)] mt-1 bg-[var(--color-primary-gold)] px-2 py-0.5 rounded-lg border-2 border-[var(--color-ink-black)]">نقطة</span>
            </div>
          </div>
        ))}
      </div>
